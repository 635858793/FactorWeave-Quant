...
try:
    logger.info("开始测试matplotlib导入（带QApplication）...")
    
    # 测试1：创建QApplication
    logger.info("步骤1: 创建QApplication...")
    try:
        from PyQt5.QtWidgets import QApplication
        app = QApplication(sys.argv)
        logger.info("✓ QApplication创建成功")
    except Exception as e:
        logger.error(f"✗ QApplication创建失败: {e}")
        traceback.print_exc()
        raise
    
    # 测试2：导入matplotlib
    logger.info("步骤2: 导入matplotlib...")
    try:
        import matplotlib
    ...
    # 测试3：设置matplotlib后端
    logger.info("步骤3: 设置matplotlib后端...")
    try:
        matplotlib.use('Agg')
    ...
    # 测试5：导入FigureCanvas
    logger.info("步骤5: 导入FigureCanvas...")
    try:
