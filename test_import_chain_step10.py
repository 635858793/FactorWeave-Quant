...
# -*- coding: utf-8 -*-
"""
测试导入链 - 逐步导入 core.coordinators.base_coordinator
"""
...
logger.info("4. 导入 core.coordinators.base_coordinator (import)...")
try:
    import core.coordinators.base_coordinator
    logger.info("✓ core.coordinators.base_coordinator (import) 导入完成")
except Exception as e:
    logger.error(f"✗ core.coordinators.base_coordinator (import) 导入失败: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

logger.info("5. 导入 BaseCoordinator...")
try:
    from core.coordinators.base_coordinator import BaseCoordinator
    logger.info("✓ BaseCoordinator 导入完成")
except Exception as e:
    logger.error(f"✗ BaseCoordinator 导入失败: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
