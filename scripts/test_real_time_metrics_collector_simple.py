...
# 配置日志输出到文件
logger.add("real_time_metrics_collector_test.log", rotation="10 MB")
...
def test_real_time_metrics_collector():
    ...
    logger.info(f"  同步收集高优先级指标时间：{high_time:.4f}秒")
    logger.info(f"  同步收集中等优先级指标时间：{medium_time:.4f}秒")
    logger.info(f"  同步收集所有指标时间：{all_time:.4f}秒")
