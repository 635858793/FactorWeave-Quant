...
async def test_initialization():
    ...
    try:
        ...
        print("测试智能推荐面板的初始化逻辑...")
        print("=" * 60)
        
        # 1. 创建推荐引擎
        print("\n1. 创建推荐引擎...")
        engine = SmartRecommendationEngine()
        print(f"✅ 推荐引擎创建成功，内容项数量: {len(engine.content_items)}")
        
        # 2. 模拟 _initialize_recommendation_engine 方法
        print("\n2. 模拟 _initialize_recommendation_engine 方法...")
        ...
        recommendations = await engine.get_recommendations(
            user_id='test_user',
            recommendation_type=RecommendationType.STOCK_A,
            count=5
        )
...
if __name__ == '__main__':
    sys.exit(asyncio.run(test_initialization()))
