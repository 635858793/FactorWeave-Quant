...
# -*- coding: utf-8 -*-
"""
测试matplotlib后端导入（跳过get_backend）
"""
...
try:
    logger.info("开始测试matplotlib后端导入（跳过get_backend）...")
    ...
    # 测试2：设置matplotlib后端为Agg
    logger.info("步骤2: 设置matplotlib后端为Agg...")
    try:
        matplotlib.use('Agg')
    ...
    # 测试5：导入FigureCanvas（不使用Qt后端）
    logger.info("步骤5: 导入FigureCanvas（不使用Qt后端）...")
    try:
