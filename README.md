<div align="center">
  ...
  <h3>FactorWeave-Quant - 因子编织量化交易系统</h3>
</div>

[![Python 3.11+](https://img.shields.io/badge/python-3.11+-blue.svg)](https://www.python.org/downloads/) [![License](https://img.shields.io/badge/license-AGPL%203.0-blue.svg)](LICENSE) [![Status](https://img.shields.io/badge/status-Active%20Development-brightgreen.svg)]()
...
**注意：学习使用请加微信，研究使用请加星球有详细的使用说明和技术支持。**
...
|---------|---------|---------|---------|---------|
| **系统启动时间** | 4.6秒 | 16.8秒 | +72.6% | 🟢 优秀 |
| **运行内存占用** | 440.0MB | 547.6MB | +19.7% | 🟡 接近达标 |
| **峰值内存使用** | 456.0MB | 549.1MB | +17.0% | 🟢 优秀 |
| **API响应时间** | 1.5ms | 30.1ms | +95.0% | 🟢 优秀 |

### 量化交易专项性能

| **量化指标** | **测试结果** | **行业标准** | **状态** |
|---------|---------|---------|---------|
| **回测速度** | ~50.0万条数据/秒 | 50万条数据/秒 | 🟢 达标 |
| **策略执行延迟** | ~0.5ms | 50ms | 🟢 优秀 |
| **数据处理吞吐量** | ~5000.0笔/秒 | 1000笔/秒 | 🟢 优秀 |

### 📊量化交易回测性能
| 数据量 | 无风控速度 | 有风控速度 | 性能提升 |
|--------|-----------|-----------|---------|
| 1,000条 | 14,285条/秒 | 10,989条/秒 | 🟢 优秀 |
| 10,000条 | 50,000条/秒 | 48,076条/秒 | 🟢 优秀 |
| 50,000条 | 71,225条/秒 | 55,066条/秒 | 🟢 优秀 |
| 100,000条 | 85,905条/秒 | 98,522条/秒 | 🟢 优秀 |
| 250,000条 | 197,315条/秒 | 220,813条/秒 | 🟢 极致 |
| 500,000条 | 329,034条/秒 | 325,309条/秒 | 🟢 极致 |

> 📝 **测试说明**：以上性能数据 测试环境为Intel i7-12700K。25万条数据约对应3年的5分钟K线数据，可在1秒内完成回测。
...
- **动态策略加载**：运行时加载/卸载策略
- **风险管理**：多层级风险控制与止损机制
  - **JIT加速风控**：Numba JIT编译，毫秒级风控计算
  - **止损/止盈**：支持百分比止损、止盈，实时触发
  - **最大持有期**：自动平仓控制持仓时间
  - **交易统计**：退出原因追踪（止损/止盈/持有期）
...
1. **@auto_jit** - 通用JIT优化装饰器
   - 适用场景：通用数值计算函数
   - 特点：自动检测函数类型并选择最优编译策略
   - 性能提升：10-100倍（取决于计算复杂度）

2. **@auto_jit_array** - 数组计算优化装饰器
   - 适用场景：NumPy数组操作、向量化计算
   - 特点：针对数组操作优化，支持并行计算
   - 性能提升：20-200倍（数组操作）

3. **@auto_jit_loop** - 循环优化装饰器
   - 适用场景：包含循环的密集计算
   - 特点：优化循环结构，消除Python解释器开销
   - 性能提升：50-500倍（循环密集型计算）
...
系统提供了完整的UI监控界面，可以实时查看：
- JIT函数数量和统计
- 编译时间和缓存命中率
- 性能提升和使用情况
- JIT启用/禁用控制

详见：[algorithm_optimization_tab.py](gui/widgets/performance/tabs/algorithm_optimization_tab.py)

**相关文档**：
- [AutoJIT使用指南](docs/AUTO_JIT_GUIDE.md)
- [JIT配置管理](docs/JIT_CONFIG.md)
- [性能优化最佳实践](docs/PERFORMANCE_OPTIMIZATION.md)
