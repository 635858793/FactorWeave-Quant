...
def test_model_trainer_initialization():
    ...
    try:
        ...
        if not container.is_registered(RecommendationModelTrainer):
            logger.error("❌ RecommendationModelTrainer 未注册")
            return False
        ...
        for check_name, check_result in checks:
            ...
            logger.info(f"{status} {check_name}: {'存在' if check_result else '不存在'}")
            if not check_result:
                all_passed = False
        
        return all_passed
    except Exception as e:
        logger.error(f"测试模型训练器初始化失败: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False
...
def test_explanation_generator_initialization():
    ...
    try:
        ...
        if not container.is_registered(RecommendationExplanationGenerator):
            logger.error("❌ RecommendationExplanationGenerator 未注册")
            return False
        ...
        logger.error(f"测试推荐理由生成器初始化失败: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False
...
def test_recommendation_engine_integration():
    ...
    try:
        ...
        if not container.is_registered(SmartRecommendationEngine):
            logger.error("❌ SmartRecommendationEngine 未注册")
            return False
        ...
        if recommendation_engine.explanation_generator is None:
            logger.error("❌ 推荐引擎的 explanation_generator 为 None")
            return False
        
        if recommendation_engine.explanation_generator != explanation_generator:
            logger.error("❌ 推荐引擎的 explanation_generator 不是注册的实例")
            return False
        
        logger.info("✅ 推荐引擎的 explanation_generator 已正确设置")
        ...
        logger.info(f"✅ 生成的推荐理由: {explanation}")
        
        return True
    except Exception as e:
        logger.error(f"测试推荐引擎集成失败: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False
...
def test_smart_recommendation_panel_creation():
    ...
    try:
        ...
        try:
            recommendation_engine = container.resolve(SmartRecommendationEngine)
            logger.info("✅ 成功获取 SmartRecommendationEngine 服务")
        except Exception as e:
            logger.warning(f"⚠️ 无法获取 SmartRecommendationEngine 服务: {e}")
        
        try:
            model_trainer = container.resolve(RecommendationModelTrainer)
            logger.info("✅ 成功获取 RecommendationModelTrainer 服务")
        except Exception as e:
            logger.warning(f"⚠️ 无法获取 RecommendationModelTrainer 服务: {e}")
        ...
        for check_name, check_result in checks:
            status = "✅" if check_result else "❌"
            logger.info(f"{status} {check_name}: {'已设置' if check_result else '未设置'}")
            if not check_result:
                all_passed = False
        ...
        if hasattr(panel, 'train_model_btn'):
            ...
            logger.info(f"✅ 模型训练按钮状态: {'可用' if is_enabled else '不可用'}")
        
        return all_passed
    except Exception as e:
        logger.error(f"测试智能推荐面板创建失败: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False
