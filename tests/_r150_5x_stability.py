"""
R150 5x 稳定性回归测试脚本
- 测试: test_r150_p1_1_risk_control_exc_info + test_r150_p1_1_unified_engine_logger_warning
...
for i in range(1, 6):
    ...
    stability = "✅" if result.returncode == 0 and failed == 0 else "❌"
...
# 汇总
print("\n" + "=" * 60)
print("R150 5x 稳定性回归汇总")
print("=" * 60)
total_passed = sum(r["passed"] for r in results)
total_failed = sum(r["failed"] for r in results)
all_stable = all(r["stability"] == "✅" for r in results)
print(f"总通过: {total_passed}")
print(f"总失败: {total_failed}")
print(f"5x 稳定性: {'✅ 100% 稳定' if all_stable else '❌ 不稳定'}")
print(f"综合评级: {'🟢 GREEN' if all_stable and total_failed == 0 else '🔴 RED'}")
