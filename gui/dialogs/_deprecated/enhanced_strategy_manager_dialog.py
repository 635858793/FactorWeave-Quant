...
合并说明：
此文件的功能已整合到统一策略管理对话框 (V4) 中：
- 基础策略管理功能 → StrategyManagerDialog 的"策略库"视图
- 回测功能 → StrategyManagerDialog 的"回测实验室"视图  
- 参数优化功能 → StrategyManagerDialog 的"参数优化"视图
- 性能分析功能 → StrategyManagerDialog 的"性能分析"视图

保留原因：向后兼容性，现有代码引用不会破坏。

迁移指南：
    # 旧代码
    from gui.dialogs import EnhancedStrategyManagerDialog
...
class EnhancedStrategyManagerDialog(BaseDialog):
    """增强策略管理对话框 V2 - 对标行业软件，集成系统主题管理"""
    ...
    def __init__(self, parent=None, strategy_service=None):
        ...
        super().__init__(
            ...
            title="策略管理器",
            size=(1600, 1000),
            settings_key="EnhancedStrategyManagerDialog",
            modal=False
        )
    ...
    def _update_optimization_chart(self, history: List[Dict[str, Any]]):
        ...
        try:
            ...
            # 标记最佳点
            if values:
                best_idx = max(enumerate(values), key=lambda x: x[1])[0]
