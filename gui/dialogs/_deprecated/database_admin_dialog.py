...
import warnings
warnings.warn(
    "DatabaseAdminDialog is deprecated and will be removed in a future version. "
    ...
    def save_description(self, database_path, database_type, table_name, description, tags=''):
        try:
            ...
            try:
                ...
                ON CONFLICT(database_path, database_type, table_name) DO UPDATE SET
...
class DatabaseAdminDialog(BaseDialog):
    def __init__(self, db_path, parent=None, mode='admin'):
        super().__init__(
            parent,
            title="数据库管理后台",
            min_size=(1300, 800),
            settings_key="DatabaseAdminDialog",
        )
    ...
    def _create_duckdb_table_model(self, schema_result, data_result, total_rows):
        ...
        class DuckDBTableModel(QAbstractTableModel):
            ...
            def _execute_filter(self, conditions):
                """执行过滤逻辑"""
                matched_rows = []
                header_index = {name: idx for idx, name in enumerate(self.headers)} if self.headers else {}
                ...
                for row_idx, row_data in enumerate(self._data):
                    ...
                    for and_conditions in conditions:
                        ...
                        for condition in and_conditions:
                            if not self._check_condition(row_data, condition, header_index):
                                row_matches = False
                                break
            ...
            def _check_condition(self, row_data, condition, header_index=None):
                """检查单个条件是否匹配"""
                ...
                try:
                    ...
                    if condition_type == 'LIKE':
                        ...
                        for not_cond in not_conditions:
                            if self._check_condition(row_data, not_cond, header_index):
                                return False
                        return True
