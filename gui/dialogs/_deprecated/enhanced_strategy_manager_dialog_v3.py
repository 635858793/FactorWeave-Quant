...
合并说明：
此文件的现代化UI组件功能已整合到统一策略管理对话框 (V4) 中：
- 可折叠侧边栏导航 → StrategyManagerDialog 的侧边栏
- 增强统计卡片 → StrategyManagerDialog 的首页统计卡片
- 可折叠面板 → StrategyManagerDialog 的首页折叠面板
- 快捷操作面板 → StrategyManagerDialog 的快捷操作
- 实时数据指示器 → StrategyManagerDialog 的实时指示器
- 工作区管理 → StrategyManagerDialog 的工作区管理器

保留原因：向后兼容性，现有代码引用不会破坏。

迁移指南：
    # 旧代码
    from gui.dialogs import EnhancedStrategyManagerDialogV3
    dialog = EnhancedStrategyManagerDialogV3(...)
...
class EnhancedStrategyManagerDialogV3(BaseDialog):
    """
    ...
    def __init__(self, parent=None, strategy_service=None):
        super().__init__(
            ...
            title="策略管理器 - 专业版",
            size=(1600, 1000),
            settings_key="EnhancedStrategyManagerDialogV3",
            modal=False,
        )
