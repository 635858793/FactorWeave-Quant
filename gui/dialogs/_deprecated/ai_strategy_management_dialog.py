...
⚠️ [已废弃] 此文件已被弃用，请使用 strategy_manager_dialog.py 中的 StrategyManagerDialog (V4)。
⚠️ [DEPRECATED] This file is deprecated. Use StrategyManagerDialog from strategy_manager_dialog.py (V4) instead.

合并说明：
此文件的AI策略管理功能已整合到统一策略管理对话框 (V4) 中：
- 策略模板管理 → StrategyManagerDialog 的"AI策略"视图 (StrategyTemplateManager)
- 策略编辑器 → StrategyManagerDialog 的"AI策略"视图 (StrategyEditorWidget)
- 配置验证 → StrategyManagerDialog 的"AI策略"视图 (StrategyConfigValidator)
- AI策略CRUD → StrategyManagerDialog 的"AI策略"视图完整功能

保留原因：向后兼容性，现有代码引用不会破坏。

迁移指南：
    # 旧代码
    from gui.dialogs import AIStrategyManagementDialog
...
from PyQt5.QtWidgets import (
    QVBoxLayout, QHBoxLayout, QTabWidget, QWidget, QDialog,
...
class AIStrategyManagementDialog(BaseDialog):
    """AI选股策略管理对话框"""
    ...
    def __init__(self, parent=None, database_service=None):
        ...
        super().__init__(
            ...
            title="AI选股策略管理",
            size=(1400, 900),
            settings_key="AIStrategyManagementDialog"
        )
